		...
		struct vm_area_struct *vma, unsigned long haddr)
{
	...
	entry = mk_huge_pmd(folio_page(folio, 0), vma->vm_page_prot);
		...
		struct folio *zero_folio)
{
	...
	if (!pmd_none(*pmd))
		return;
	entry = mk_pmd(folio_page(zero_folio, 0), vma->vm_page_prot);
			...
			unsigned long dst_addr, unsigned long src_addr)
{
	...
	if (src_folio) {
		...
		_dst_pmd = mk_huge_pmd(folio_page(src_folio, 0), dst_vma->vm_page_prot);
		...
		bool uniform_split)
{
	...
	for (split_order = start_order;
		...
		split_page_memcg(folio_page(folio, 0), old_order, split_order);
				     ...
				     unsigned int new_order)
{
	...
	return __folio_split(folio, new_order, folio_page(folio, 0), page, list, true);
}
...
{
	return __folio_split(folio, new_order, split_at, folio_page(folio, 0), list,
			false);
}
...
int split_folio_to_list(struct folio *folio, struct list_head *list)
{
	...
	return split_huge_page_to_list_to_order(folio_page(folio, 0), list, ret);
}
